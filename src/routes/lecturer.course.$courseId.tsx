...
function CourseDetail() {
  ...
  useEffect(() => {
    ...
    void loadAll();
  }, [authLoading, user, role, courseId, navigate]);
  ...
  const loadAll = async () => {
    ...
    const studentIds = Array.from(
      new Set([
        ...(a ?? []).map((r) => r.student_id),
        ...enrollmentsData.map((r) => r.student_id),
      ])
    );
  ...
  const startSession = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        scan_token: scanToken,        // ← Critical fix
        starts_at: startsAt,          // ← Added
    ...
    setTopic("");
    await loadAll();
    void showQr(data as SessionRow);
  };

  const showQr = async (s: SessionRow) => {
    if (!s.scan_token) {
      toast.error("Session is missing scan token");
      return;
    }
    ...
    try {
      ...
      const dataUrl = await QRCode.toDataURL(url, {
        width: 480,
        margin: 2,
        color: { dark: "#1b1340", light: "#ffffff" },
      });
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="max-w-2xl mx-auto px-6 py-20 text-center">
          ...
          <Link to="/lecturer" className="text-primary font-medium">
            Back to dashboard
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-background">
            ...
            {sessions.length === 0 ? (
              <div className="bg-gradient-card rounded-2xl border border-border p-12 text-center shadow-soft">
                <p className="text-muted-foreground">
                  No sessions yet. Start your first session to generate a QR code.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {sessions.map((s) => {
                  ...
                  return (
                    <div
                      key={s.id}
                      className="bg-gradient-card rounded-2xl border border-border p-6 shadow-soft"
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground flex items-center gap-3">
                            ...
                            {new Date(s.starts_at).toLocaleString()} → expires{" "}
                      ...
                      <div className="border-t border-border pt-3">
                        <p className="text-sm font-medium mb-2">
                          {att.length} student{att.length === 1 ? "" : "s"} checked in
                        </p>
                        {att.length > 0 && (
          ...
          <TabsContent value="students" className="mt-6">
            <div className="bg-gradient-card rounded-2xl border border-border p-6 shadow-soft mb-4">
              ...
              <p className="text-sm text-muted-foreground mb-3">
                Share this code with your students so they can enroll from their dashboard.
              </p>
                ...
                {enrollments.map((en) => (
                  ...
                  >
                    <span className="font-medium">
                      {en.profiles?.full_name || "Student"}
                    </span>
                    <span className="text-sm text-muted-foreground">
                      {en.profiles?.university_id}
                    </span>
                  </li>
                ))}
              </ul>
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{activeQr?.session.topic}</DialogTitle>
          </DialogHeader>
          {activeQr && (
            <div className="space-y-4">
              ...
              <p className="text-center text-sm text-muted-foreground">
                Active until{" "}
                {new Date(activeQr.session.expires_at).toLocaleTimeString()}
              </p>
            </div>
          )}
        </DialogContent>
